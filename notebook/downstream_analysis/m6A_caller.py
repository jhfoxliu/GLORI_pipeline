...
def call_m6A(line,nonCRs,CR,C_cutoff,control):
	col = C_cutoff_cols.get(C_cutoff)
	chr,pos_1,dir,gene,name,trans,isoform,biotype,total_cov,CT,C_count,T_count,A_count,G_count = line[0:14] # C = A
	# print(chr,pos_1,dir,gene,name,trans,isoform,biotype,total_cov,CT,C_count,T_count,A_count,G_count)
	# print(total_cov,CT,C_count,T_count,A_count,G_count)
	...
	if CT_col >= options.coverage and C_count_col >= options.count:
		...
		if options.method == "binomial":
			...
			pvalue = scipy.stats.poisson.sf(C_count_col, int(np.ceil(nonCR*CT_col)))
...
def fetch_m6A(line,nonCRs,CR,C_cutoff,control):
	col = C_cutoff_cols.get(C_cutoff)
	chr,pos_1,dir,gene,name,trans,isoform,biotype,total_cov,CT,C_count,T_count,A_count,G_count = line[0:14] # C = A here
	# print(chr,pos_1,dir,gene,name,trans,isoform,biotype,total_cov,CT,C_count,T_count,A_count,G_count)
...
def search_for_candidates(args):
	...
	if fn.endswith(".txt"):
		with open(fn,'r') as input:
			...
			while (line):
				line = line.strip().split("\t")
				result = call_m6A(line,nonCRs,CR,cutoff,control)
				if result is not None:
					results.append(result)
				line = input.readline()
	
	elif fn.endswith(".txt.gz"):
		with gzip.open(fn,'rb') as input:
			nonCRs = {}
			results = []
			control = {}
			if CR == "control":
...
def fetch_candidates_with_list(args):
	...
	if fn.endswith(".txt"):
		with open(fn,'r') as input:
			...
			while (line):
				line = line.strip().split("\t")
				result = fetch_m6A(line,nonCRs,CR,cutoff,control)
				if result is not None:
					results.append(result)
				line = input.readline()
	...
	for records in results:
		...
		data[name]["A count"][key] = C_count_col
		data[name]["m6A level"][key] = ratio
...
if __name__ == "__main__":
	description = """
	Step1: read through all of the files, find m6A sites, and merge the list (optional)

	Step2: Use the site list (or a given one) to fetch sites
	
	Step3: Merge tables
	"""
	parser = argparse.ArgumentParser(prog="m6A_caller_multiple",fromfile_prefix_chars='@',description=description,formatter_class=argparse.RawTextHelpFormatter)
	#Require
	...
	sys.stderr.write("[%s] Searching for m6A candidates...\n" % (strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
	#Step 1

	# test
	# for i in search_list:
	# 	search_for_candidates(i)
	# 	sys.exit()
