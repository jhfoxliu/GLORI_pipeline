...
def filter(row):
	cov,C_count,pvalue,signal,nonCR = row["coverage"],row["A count"],row["P-value"],row["signal"],row["nonCR"]
...
def handling_groups(df):
	...
	for sample in samples:
		...
		subdf = subdf[(subdf["coverage"] >= options.coverage ) & (subdf["A count"]>= options.count) & (subdf["P-value"]< options.pvalue) & (subdf["signal"]>= options.signal) & (subdf["nonCR"] < options.gene_CR) & (subdf["m6A level"] >= options.ratio)]
		...
		if options.no_mask_low == False:
			...
			df.loc[indexes,(sample,"m6A level")] = np.nan
			df.loc[indexes,(sample,"A count")] = np.nan
	...
	order = {'coverage':0,'A count':1,'m6A level':2,'passed':3,'P-value':4,'signal':5,'nonCR':6}
