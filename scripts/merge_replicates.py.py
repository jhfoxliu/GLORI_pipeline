...
def subdf_formation(subs,format_df):
	if len(subs) == 1:
		...
		min_C = options.count
	for item in subs:
		not_m5C_indexes = format_df[(format_df[(item,"coverage")] >= options.coverage) & (format_df[(item,"A count")] < min_C)].index
		...
		format_df.loc[low_cover_indexes,(item,"m6A level")] = np.nan
		format_df.loc[low_cover_indexes,(item,"coverage")] = 0
		format_df.loc[low_cover_indexes,(item,"A count")] = 0
		format_df.loc[low_cover_indexes,(item,"coverage")] = 0
		...
		format_df.loc[bad_indexes,(item,"m6A level")] = np.nan
		format_df.loc[bad_indexes,(item,"coverage")] = 0
		format_df.loc[bad_indexes,(item,"A count")] = 0
		format_df.loc[bad_indexes,(item,"coverage")] = 0
...
def handling_groups(df,sample_groups):
	...
	# Reorder indexes
	# coverage	A count	m6A level	P-combined	passed	replicates
	reorder_columns = []

	for group, subs in sample_groups.items():
		subdf = df.iloc[:, df.columns.get_level_values(0).isin(subs)].copy()
		
		reorder_columns.append((group,"coverage"))
		reorder_columns.append((group,"A count"))
		reorder_columns.append((group,"m6A level"))
		reorder_columns.append((group,"P-combined"))
		reorder_columns.append((group,"passed"))
		reorder_columns.append((group,"replicates"))
		
		# for sub in subs:
			# indexes = subdf[(subdf[(sub,"coverage")]>=options.coverage_nan)&(subdf[(sub,"A count")]==0)].index
			# subdf.loc[indexes,(sub,"m6A level")] = np.nan
			# subdf.loc[indexes,(sub,"A count")] = np.nan
		...
		df_new = pd.DataFrame(columns=pd.MultiIndex.from_product([[group],["coverage","A count","m6A level","P-combined","passed","replicates"]]))
		
		#Coverages, A count and m6A level
		df_new[(group,"coverage")] = subdf.xs("coverage",axis=1,level=1).sum(axis=1)
		df_new[(group,"A count")] = subdf.xs("A count",axis=1,level=1).sum(axis=1)
		df_new[(group,"m6A level")] = df_new[(group,"A count")]/df_new[(group,"coverage")]
		...
		# df_new.loc[df_new[(group,"coverage")]<options.coverage_nan,(group,"m6A level")] = np.nan
		...
		df_new.loc[(df_new[(group,"m6A level")]<options.merged_ratio),(group,"passed")] = False
		df_new.loc[(df_new[(group,"m6A level")]<options.merged_ratio),(group,"P-combined")] = 1
		#drop not enough replicates
		sub_len = len(subs)
		
		# df_new.loc[df_new[(group,"A count")]<options.count*df_new[(group,"present")],(group,"passed")] = False
		df_new.loc[(df_new[(group,"passed")]==True) & ((df_new[(group,"replicates")] == 1))& (df_new[(group,"A count")] < options.single_count),(group,"passed")] = False
	...
	df.index.names = [("INFO","chr"),("INFO", "pos"),("INFO", "strand")]
...
if __name__ == "__main__":
	...
	group_sample.add_argument("-C","--count",dest="count",default=3,type=int,help="minimal A count for each site, default=3")
	group_sample.add_argument("--single-C",dest="single_count",default=5,type=int,help="minimal A count for sites without replicates, default=5")
	...
	# group_merged.add_argument("--merged-count",dest="merged_count",type=float,help="minimal ratio for merged site, if not given, equals minimal replicate * minimal A count")
